      ...
      <div className="max-w-7xl mx-auto">
        ...
        <Card className="mb-6 text-white">
          <CardHeader>
            ...
            <p className="text-gray-400">Vos livraisons à venir organisées par jour</p>
          ...
          <Card className="bg-[#1a1a1a] border-gray-800">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-white">{PLANNED_TRIPS.length}</p>
                  <p className="text-sm text-gray-400">Trajets planifiés</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">Commandes à livrer</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-[#1a1a1a] border-gray-800">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-white">{Object.keys(groupedTrips).length}</p>
                  <p className="text-sm text-gray-400">Jours de livraison</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-6">
          {Object.entries(groupedTrips).map(([date, trips]) => (
            <Card key={date} className="bg-[#1a1a1a] border-gray-800">
              <CardHeader className="bg-blue-500/10 border-b border-gray-800">
                <CardTitle className="flex items-center gap-3">
                  <Calendar className="w-6 h-6 text-blue-400" />
                  <span className="capitalize text-white">{formatDate(date)}</span>
              ...
              <CardContent className="p-6">
                <div className="space-y-4">
                  {trips.map((trip) => (
                    <Card
                      key={trip.id}
                      className={`border-l-4 border-l-blue-500 transition-all hover:shadow-lg cursor-pointer bg-[#0f0f0f] border-gray-800 ${selectedTrip?.id === trip.id ? 'ring-2 ring-blue-500' : ''
                        }`}
                      ...
                      <CardContent className="p-5">
                        <div className="space-y-4">
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex items-center gap-3">
                              <Badge variant="outline" className="bg-blue-500/20 font-mono border-blue-500/30 text-blue-400">
                                {trip.id}
                              </Badge>
                              <div>
                                <p className="font-semibold text-lg text-white">{trip.client.name}</p>
                                <p className="text-sm text-gray-400">{trip.client.phone}</p>
                              </div>
                            </div>
                            <Badge className="bg-blue-500/20 text-blue-400 font-semibold border border-blue-500/30">
                              {trip.time}
                            </Badge>
                          </div>
                          ...
                          <div className="grid md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-3">
                              <div className="flex items-center gap-3 bg-[#1a1a1a] p-3 rounded-lg border border-gray-800">
                                <Clock className="w-5 h-5 text-gray-400" />
                                <div>
                                  <p className="text-xs text-gray-500">Durée estimée</p>
                                  <p className="text-sm font-semibold text-gray-300">{trip.estimatedDuration}</p>
                                </div>
                              </div>
                          ...
                          {selectedTrip?.id === trip.id && (
                            <div className="pt-4 border-t border-gray-800 space-y-3 animate-in fade-in duration-300">
